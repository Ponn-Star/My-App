        ...
        <div className='py-28 md:py-35 px-4 md:px-16 lg:px-24 xl:px-32'>
            ...
            <div className='flex flex-col lg:flex-row mt-6 gap-6'>
                <div className='lg:w-1/2 w-full'>
                    <img src={imageMap[mainImage] || assets[mainImage] || mainImage} alt="Room Image" className='w-full rounded-xl shadow-lg object-cover' />
                </div>
                <div className='grid grid-cols-2 gap-4 lg:w-1/2 w-full'>
                    {room?.images.length > 1 && room.images.map((img, idx) => (
                        <img key={idx} src={imageMap[img] || assets[img] || img} alt='Room Image' className={`w-full rounded-xl shadow-md object-cover cursor-pointer ${mainImage === img && 'outline-3 outline-orange-500'}`} onClick={() => setMainImage(img)} />
                    ))}
                </div>
            </div>
