...
const RoomDetails = () => {
    ...
    // Gọi API lưu loại phòng vừa xem vào recentSearchedRoomType
    useEffect(() => {
        const saveRecentRoomType = async () => {
            if (room && user) {
                try {
                    await axios.post(
                        '/api/user/store-recent-search',
                        { recentSearchedRoomType: room.roomType },
                        { headers: { Authorization: `Bearer ${await getToken()}` } }
                    );
                } catch (error) {
                    // Không cần toast, chỉ log nếu cần debug
                    // console.error("Lưu recentSearchedRoomType thất bại", error);
                }
            }
        };
        saveRecentRoomType();
    }, [room, user, axios, getToken]);
